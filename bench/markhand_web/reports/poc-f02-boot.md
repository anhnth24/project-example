...
> **Evidence-quality note:** the narrative below is still hand-authored / [Unverified].
> The evidence **harness** is now hardened (`poc-boot-evidence.sh` +
> `poc_f02_boot_evidence.py --self-test`: O04 `composeProject`/`imageIds`, allowlisted
> inspect, secret scan, executable egress probe, nonzero limits). Live machine evidence
> must still be regenerated on a standard Docker host (not nested vfs/nolimit) via
> `POC_EVIDENCE_RAW_DIR=bench/markhand_web/reports/phase-1b-gate/raw/f02-<git> deploy/scripts/poc-boot-evidence.sh`
> and committed before claiming Done. Do not treat the PASS table below as that regeneration.
