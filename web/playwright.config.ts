...
//
// Real mode is serial with traces/screenshots/video off so credential-bearing
// UI dumps cannot leak into retained artifacts (P2-20 base policy).

/** Build the Playwright config from an env map (exported for unit tests). */
export function createPlaywrightConfig(env: NodeJS.ProcessEnv = process.env): PlaywrightTestConfig {
