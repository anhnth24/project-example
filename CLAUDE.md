...
- **`crates/core`** (`fileconv-core`) — lõi convert. `Converter::convert_path(&Path)` định
  ...
  - `audio.rs` — `AudioEngine::load()` lấy `WhisperContext` từ cache **process-wide**
 (key: canonical path + load knobs); decode mp3/wav/ogg… bằng `symphonia` + resample 16kHz
 (downsample có anti-alias nhẹ), phiên âm whisper-rs (lang "vi"), lọc
...
- PDF/whisper resource đắt → giữ pattern cache (thread_local PDFium, process-wide Whisper
  `LoadOnceCache` trong `audio.rs` — không reload model mỗi `Converter`/request).
