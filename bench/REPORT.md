...
|---|---|---|
| pdf | `pdfium-render` (chính) + `pdf-extract` (dự phòng) | PDFium bền, không panic, nhanh ~3× |
...
| 5 | `pdf-extract` **panic** + trích thiếu text | chuyển sang **PDFium** (cache 1 instance/thread), fallback pdf-extract | 10/10 không panic, **nhanh 3×**, text đầy đủ hơn (vd 1409.1556: 656→55.605 ký tự) |
...
| html | 10 | 10 | **14.93** | — | 25 723 |
| pdf  | 10 | 10 | **78.77** | **5.67** | 26 075 |

**100% (60/60) file convert thành công.** So với bản bọc markitdown-rs: **html nhanh
hơn ~7×** (15ms vs 132ms) và hết phình; **pdf nhanh hơn ~3×** sau khi chuyển sang
PDFium (5.67 ms/trang vs 17.6) và trích text đầy đủ hơn.
...
- **OCR chữ viết tay**: Tesseract không hợp (47.9% trên mẫu mô phỏng). Cần engine khác
  ...
  đo chính xác.*
- **PDF**: PDFium (bền, nhanh) — cần `libpdfium` (`bench/download_pdfium.sh`, đặt ở
  `./pdfium/lib` hoặc trỏ `FILECONV_PDFIUM_LIB`); thiếu thì fallback `pdf-extract`.
  Quyết định **theo từng trang**: trang có text → trích trực tiếp; trang scan/ảnh →
  render + OCR Tesseract. Trang **trộn** (text + ảnh chứa chữ): bật `pdf_ocr_images`
  để OCR thêm ảnh nhúng lớn (mặc định tắt vì OCR mọi hình trong tài liệu thường gây
  chậm/nhiễu). PDF scan tiếng Việt đã kiểm thử: render+OCR ~1.2 s/trang, ra đúng text.
...
bash bench/download_pdfium.sh                  # thư viện PDFium (trích PDF)
cargo build --release
