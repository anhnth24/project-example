...
- **OCR chữ viết tay**: Tesseract không hợp (47.9% trên mẫu mô phỏng). Cần engine khác
  ...
  đo chính xác.*
- **PDF**: đã dùng PDFium (bền, nhanh) — cần thư viện `libpdfium` (tải bằng
  `bench/download_pdfium.sh`, đặt ở `./pdfium/lib` hoặc trỏ `FILECONV_PDFIUM_LIB`);
  nếu thiếu sẽ tự fallback `pdf-extract`. PDF scan ảnh (không có text layer) vẫn cần OCR (chưa làm).
